...
def main():
    ...
    while cap.isOpened() and cap2.isOpened():
        ...
        if len(parsed_images) >= 2:
            ...
            # traffic_convention_data.resize((1, 2), refcheck=False)
            ...
            if counter == end_counter - 2:
                ...
                plt.title("Road lines" + str(counter))
                plt.xlabel("red - road lines | green - predicted path | yellow - lane lines")
                plt.ylabel("Range")
                plt.show()
                plt.pause(0.1)
                plt.clf()
            results.append(lane_lines_prob[0])
        if counter == end_counter - 2:
            frame = cv2.resize(frame, (900, 500))
            frame2 = cv2.resize(frame2, (900, 500))
